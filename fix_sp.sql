...
BEGIN
    ...
    SELECT 
        ...
        CASE WHEN ((ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0) - ISNULL(SUM(vd.Cantidad * ISNULL(vd.PrecioCompra, 0)), 0)) / NULLIF(ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0), 0)) * 100 >= 30 THEN 'ALTA' WHEN ((ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0) - ISNULL(SUM(vd.Cantidad * ISNULL(vd.PrecioCompra, 0)), 0)) / NULLIF(ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0), 0)) * 100 >= 15 THEN 'MEDIA' ELSE 'BAJA' END AS Rentabilidad,
        CASE WHEN ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0) - ISNULL(SUM(vd.Cantidad * ISNULL(vd.PrecioCompra, 0)), 0) < 0 THEN 'Ajustar precio de venta o reducir costo' WHEN ((ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0) - ISNULL(SUM(vd.Cantidad * ISNULL(vd.PrecioCompra, 0)), 0)) / NULLIF(ISNULL(SUM(vd.Cantidad * vd.PrecioVenta), 0), 0)) * 100 < 10 THEN 'Margen muy bajo, revisar estrategia' WHEN ISNULL(SUM(vd.Cantidad), 0) = 0 AND ISNULL((SELECT SUM(lp.CantidadDisponible) FROM LotesProducto lp WHERE lp.ProductoID = p.ProductoID AND lp.Estatus = 1), 0) > 0 THEN 'Sin ventas en el periodo, promover o descontar' ELSE 'Mantener estrategia actual' END AS Recomendacion
    FROM Productos p
