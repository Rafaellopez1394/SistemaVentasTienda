...
(function ($, balor, amplify) {
    ...
    $('body').on('click', '#BotonImprimir', null, function () {
        ...
        if (ValidaImprimir()) {
            ...
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=EstadoResultados" + parametros, 'w_PopBalanceGeneral' + $("#fechaInicio").val().trim().replace("/", "") + $("#fechaFinal").val().trim().replace("/", "") + $('#ddl-empresa option:selected').val(), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
        }
    });
