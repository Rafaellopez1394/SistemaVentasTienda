(function ($, balor, amplify) {
    ...
    var traerVendedoresCombustible = function () {
        var vendedorid = $('#ayudaVendedor_ValorID').val();
        var estatus = $("#cboEstatus option:selected").val();
        ejecutaAjax("AdministracionCombustible.aspx/TraerVendedoresCombustible", "POST", { "vendedorid": vendedorid, "estatus": estatus },
            function (d) {
                if (d.d.EsValido) {
                    var datos = d.d.Datos;
                    var h = "";
                    for (var i = 0; i < datos.length; i++) {
                        h += "<tr data-vendedorcombustibleid='" + datos[i].Vendedorcombustibleid + "' data-vendedorid='" + datos[i].Vendedorid + "'>";
                        h += "<td>" + datos[i].Nombre + "</td>";                        
                        h += "<td align='center'>" + datos[i].Ubicacionlatitud + "</td>";
                        h += "<td align='center'>" + datos[i].Ubicacionlongitud + "</td>";
                        if (datos[i].Estatus == 1) {
                            h += "<td align='center'>ACTIVO</td>";
                        }
                        else {
                            h += "<td align='center'>INACTIVO</td>";
                        }
                        
                        h += "<td align='center'>";
                        if (datos[i].Estatus == 1) {                            
                            h += "<span class='baja' title='Dar de baja' style='cursor:pointer'><i class='fa fa-times-circle'></i></span>"
                        }
                        if (datos[i].Estatus == 2) {
                            h += "<span class='alta' title='Activar vendedor' style='cursor:pointer'><i class='fa fa-check-circle'></i></span>"
                        }
                        h += "<span class='mapa' title='Ver localización'><i class='fa fa-map-marker' aria-hidden='true'></i></span>"
                        h += "</td>";
                        h += "</tr>";
                    }
                    ...
                    $("#tblSolicitudes tbody tr").each(function () {
                        $(this).find("td:eq(" + colOpciones + ")").find(".baja").click(function () {
                            baja(this);
                        });

                        $(this).find("td:eq(" + colOpciones + ")").find(".alta").click(function () {
                            activar(this);
                        });
    ...
    var baja = function (elem) {
        if (!confirm("¿Desea dar de baja a vendedor?")) {
            return;
        }
        var vendedorid = $(elem).closest("tr").attr("data-vendedorid");
        ejecutaAjax("AdministracionCombustible.aspx/BajaVendedor", "POST", { "vendedorid": vendedorid, "usuario": getUsuario() },
            function (d) {
                if (d.d.EsValido) {
                    showMsg("Baja de vendedor realizado con éxito", "", "success");
                    traerVendedoresCombustible();
                }
                else {
                    showMsg("Alerta", d.d.Mensaje, "warning");
    ...
    var activar = function (elem) {
        if (!confirm("¿Desea dar de baja a vendedor?")) {
            return;
        }
        var vendedorid = $(elem).closest("tr").attr("data-vendedorid");
        ejecutaAjax("AdministracionCombustible.aspx/ActivarVendedor", "POST", { "vendedorid": vendedorid, "usuario": getUsuario() },
            function (d) {
                if (d.d.EsValido) {
                    showMsg("Baja de vendedor realizado con éxito", "", "success");
                    traerVendedoresCombustible();
