...
(function ($, balor, amplify) {
    ...
    $('body').on('click', '#BotonImprimir', null, function () {
        let firma = document.querySelector('input[name="cfirma"][value="1"]').checked;
        if (ValidaImprimir()) {
            ...
            parametros += "&cuentasDolares=" + ($("#chkCuentaDolares").is(":checked") ? 1 : 0);
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=BalanceGeneral" + parametros, 'w_PopBalanceGeneral' + $("#fechaInicio").val().trim().replace("/", "") + $("#fechaFinal").val().trim().replace("/", "") + $('#ddl-empresa option:selected').val(), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
        }
    });
    let wasChecked = false;

    document.querySelector('input[name="cfirma"][value="1"]').addEventListener('click', function (event) {
        if (wasChecked) {
            this.checked = false;  
            wasChecked = false;
        } else {
    ...
    document.querySelector('input[name="cfirma"][value="1"]').addEventListener('change', function () {
