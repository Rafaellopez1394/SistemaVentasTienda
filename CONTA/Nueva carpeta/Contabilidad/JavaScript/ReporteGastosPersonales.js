    ...
    */
    ...
    $('body').on('click', '#BotonImprimir', null, function () {
        if (ValidaImprimir()) {
            ...
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=GastosPersonales" + parametros, 'w_PopGastosPersonales' + $("#fechaInicio").val().trim().replace("/", "") + $("#fechaFinal").val().trim().replace("/", "") + $('#ddl-empresa option:selected').val() + $('#AyudaCuentaIncial').getValuesByCode().Codigo.trim() + $('#AyudaCuentaFinal').getValuesByCode().Codigo.trim(), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
        }
    });
    $('body').on('click', '#btnLimpiar', null, function () {
        window.location.replace("ReporteGastosPersonales.aspx");
    });
    ...
    $(document).ready(function () {
        ...
        AplicarEstilo();
        iniciaAyudas();
        ejecutaAjax('ReporteGastosPersonales.aspx/TraerEmpresas', 'POST', null, llenarComboEmpresas, null, false);
    });
