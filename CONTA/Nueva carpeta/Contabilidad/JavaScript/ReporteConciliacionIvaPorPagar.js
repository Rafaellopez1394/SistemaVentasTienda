(function ($, balor, amplify) {
    ...
    */
    ...
    var ValidaImprimir = function () {
        if ($("#fechaini").val() == "") {
            showMsg('Alerta', "Debe ingresar la fecha inicial del rango", 'warning');
            $("#fechaini").focus();
            return false;
        }
        if ($("#fechafin").val() == "") {
            showMsg('Alerta', "Debe ingresar la fecha final del rango", 'warning');
            $("#fechafin").focus();
            return false;
        }
        return true;
    ...
    $('body').on('click', '#BotonImprimir', null, function () {
        if (ValidaImprimir()) {
            var parametros = "&fechaini=" + $("#fechaini").val().trim();
            parametros += "&fechafin=" + $("#fechafin").val().trim();
            parametros += "&empresaid=" + $('#ddl-empresa option:selected').val();
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=ReporteConciliacionIvaPorPagar" + parametros, 'w_PopRelacionSaldos' + $("#fechaini").val().trim().replace("/", ""), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
        }
    });
