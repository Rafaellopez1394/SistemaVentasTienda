/// <reference path="../../Ba   se/js/vendor/jquery-1.11.0-vsdoc.js" />
...
(function ($, balor, amplify) {
    ...
    $('body').on('click', '#BotonImprimir', null, function () {
        if (ValidaImprimir()) {
            ...
            parametros += "&TipoReporte=" + $("input[type='radio'][name='timprimir']:checked").val(); //balor.radioGetCheckedValue('');
            parametros += "&IncluirDemandados=" + ($("#ckIncluirDemandados").prop('checked') ? 1 : 0);
            parametros += "&SoloDemandados=" + ($("#ckSoloDemandados").prop('checked') ? 1 : 0);
            parametros += "&FormatoImprimir=" + $("input[type='radio'][name='Formatoimprimir']:checked").val();
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=ConciliacionClientes" + parametros, 'w_ConciliacionClientes' + $('#ddl-empresa option:selected').val(), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
        }
    });
    ...
    $('body').on('click', '#ckSoloDemandados', null, function () {
        $("#ckIncluirDemandados").prop('checked', false);
    });

    $('body').on('click', '#ckIncluirDemandados', null, function () {
        $("#ckSoloDemandados").prop('checked', false);
    });

    $('body').on('click', '#btnLimpiar', null, function () {
        window.location.replace('ConciliacionClientes.aspx');
    });
