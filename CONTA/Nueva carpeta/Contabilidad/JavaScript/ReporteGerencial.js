...
//    });
    $('body').on('click', '#BotonImprimir', null, function () {
        if (ValidaImprimir()) {
            ...
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=GerencialReport" + parametros, 'w_PopReporteGerencial' + $("#fechaInicio").val().trim().replace("/", "") + $("#fechaFinal").val().trim().replace("/", "") + $('#ddl-empresa option:selected').val(), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');

            if ($('#chkAnexoCapitalInvertido').is(':checked')) {
                window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=AnexoCapitalInvertido" + parametros, 'w_PopReporteGerencialAnexoCI' + $("#fechaInicio").val().trim().replace("/", "") + $("#fechaFinal").val().trim().replace("/", "") + $('#ddl-empresa option:selected').val(), 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
            }

        }
    });
    $('body').on('click', '#btnLimpiar', null, function () {
        window.location.replace("ReporteGerencial.aspx");
    });
