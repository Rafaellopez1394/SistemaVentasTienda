...
(function ($, balor, amplify) {
    ...
    function procesarXMLComplemento(files) {
        ...
        xhr.open('POST', 'ProcesarComplementoClientes.ashx');
        xhr.onload = function () {
    ...
    var ProcesarXML = function (Datos) {
        ...
        for (var i = 0; i < Datos.length; i++) {
            ...
            resHtml += '<td style="text-align:center;">' + Datos[i].Factura + '</td>';
            resHtml += '<td style="text-align:center;">' + formatoFechaJSON(Datos[i].Fechatimbrado) + '</td>';
            resHtml += '<td style="text-align:center;">' + Datos[i].NoComplementos + '</td>';
    ...
    $('body').on('click', '.elim-Factura', null, function () {
        if (confirm('¿Desea eliminar esta factura?')) {
            ...
            if (fp != 0) {
                ...
                ejecutaAjax('CapturaComplementoPagoCliente.aspx/EliminarComplemento', 'POST', parametros, function (d) {
                    if (d.d.EsValido) {
                        tr.remove();
                    } else {
    ...
    var guardar = function () {
        ...
        ejecutaAjax('CapturaComplementoPagoCliente.aspx/Guardar', 'POST', { value: JSON.stringify(parametros) }, function (d) {
            ShowLightBox(false, "");
            if (d.d.EsValido) {
                //showMsg('Guardado', 'Guardado correctamente', 'success');
                alert('Guardado correctamente.');
                window.location.replace("CapturaComplementoPagoCliente.aspx");
    ...
    var GetComplementos = function () {
        ...
        $("#TablaComplementos tbody tr").each(function () {
            ...
            if (Facturapagoid == 0) {
                ...
                Catfacturaspago.Clienteid = $(this).attr("data-Clienteid");
                Catfacturaspago.ClienteRFC = $(this).attr("data-rfcemisor");
