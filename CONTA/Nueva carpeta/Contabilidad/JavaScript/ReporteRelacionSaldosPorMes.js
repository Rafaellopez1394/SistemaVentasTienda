...
(function ($, balor, amplify) {
    ...
    */
    ...
    var TraerUltimaCuentaContable = function (cuenta) {
        ...
        parametros.empresaid = ($('#ddl-empresa option:selected').val() == "" ? "FA764836-BB07-4EB3-9B30-2B69206174C2" : $('#ddl-empresa option:selected').val());
    ...
    var AyudaCuentaIncial_FindByCode = function () {
        ...
        parametros.ID = ($('#ddl-empresa option:selected').val() == "" ? "FA764836-BB07-4EB3-9B30-2B69206174C2" : $('#ddl-empresa option:selected').val());
    ...
    var ValidaImprimir = function () {
        if ($("#txtEjercicio").val() == "") {
            showMsg('Alerta', "Debe ingresar el ejercicio a consultar", 'warning');
            $("#fechaInicio").focus();
            return false;
        }
        if ($("#fechaFinal").val() == "") {
    ...
    $('body').on('blur', '#fechaFinal', null, function () {
        var ffecha = $(this).val();
    ...
    $("input[type='radio'][name='SoloGastosContables']").on('click', function (event) {
        // Si el radio button estaba previamente seleccionado
        if ($(this).is(':checked') && previousChecked === $(this).val()) {
            // Desmarcar el radio button
            $(this).prop('checked', false);
            previousChecked = null;
            event.stopImmediatePropagation();
        } else {
            // Recordar el nuevo radio button seleccionado
            previousChecked = $(this).val();
        }
    });
    ...
    $('body').on('click', '#BotonImprimir', null, function () {
        if (ValidaImprimir()) {
            var parametros = "&fechaFinal=" + $("#fechaFinal").val().trim();
            parametros += "&empresaid=" + $('#ddl-empresa option:selected').val();
            parametros += "&cuentaInicial=" + $('#AyudaCuentaIncial').getValuesByCode().Codigo.trim();
            parametros += "&cuentaFinal=" + $('#AyudaCuentaFinal').getValuesByCode().Codigo.trim();
            parametros += "&ejercicio=" + $("#txtEjercicio").val().trim();
            parametros += "&gastosPersonales=" + ($("input[type='radio'][name='SoloGastosContables']").prop('checked') ? 1 : 0);
            parametros += "&FormatoImprimir=" + $("input[type='radio'][name='FormatoImprimir']:checked").val();
            window.open("../../Base/Formas/ReporteProduccion.aspx?Formato=0&Nombre=RelacionSaldosPorMes" + parametros, 'w_PopRelacionSaldosPorMes', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=760,height=560');
        }
    });

    $('body').on('click', '#btnLimpiar', null, function () {
        window.location.replace("ReporteRelacionSaldosPorMes.aspx");
    });
