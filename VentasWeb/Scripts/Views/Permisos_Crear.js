        ...
        success: function (data) {
            ...
            if (data.data != null)
                $.each(data.data, function (i, item) {
                    ...
                    if (item.Activo == true) {
                        $("<option>").attr({ "value": item.RolID }).text(item.Descripcion).appendTo("#cboRol");
                    }
                })
        },
            ...
            if (data != undefined) {
                $.each(data, function (i, row) {
                    $("<tr>").append(
                        $("<td>").text(i + 1),
                        $("<td>").append(
                            $("<input>").attr({ "type": "checkbox" }).data("IdPermiso", row.PermisosID).prop('checked', row.Activo)
                        ),
                        $("<td>").text(row.Menu),
                        $("<td>").text(row.SubMenu)
...
function Guardar() {
    ...
    $('input[type="checkbox"]').each(function () {
        ...
        permiso = permiso + "<PERMISO><PermisosID>" + idpermiso + "</PermisosID><Activo>" + activo + "</Activo></PERMISO>";
